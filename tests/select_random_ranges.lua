...
function prepare()
   ...
   if (db_driver == "oracle") then
      db_query("CREATE SEQUENCE mgtest_seq")
      db_query([[CREATE TRIGGER mgtest_trig BEFORE INSERT ON mgtest 
                 FOR EACH ROW BEGIN SELECT mgtest_seq.nextval INTO :new.id FROM DUAL; END;]])
   end

   db_query("CREATE INDEX k on mgtest(k)")

   print("Inserting " .. oltp_table_size .. " records into 'mgtest'")
   
   if (oltp_auto_inc) then
      db_bulk_insert_init("INSERT INTO mgtest(k, c, pad) VALUES")
   else
      db_bulk_insert_init("INSERT INTO mgtest(id, k, c, pad) VALUES")
   end
...
function cleanup()
   print("Dropping table 'mgtest'...")
   db_query("DROP TABLE mgtest")
end
